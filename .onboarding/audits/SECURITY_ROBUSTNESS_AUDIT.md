...
Le thème ne contient ni formulaire, ni authentification, ni route applicative propre — la surface d'attaque locale est structurellement faible. Le principal risque de sécurité est **externe** : jQuery 1.12.4 est chargé depuis un CDN tiers sans contrôle d'intégrité (`VÉRIFIÉ_CODE` : `functions.php:19`), et cette version est associée à des vulnérabilités référencées dans les bases NVD/Mitre (source externe citée au constat détaillé, non vérifiable depuis le dépôt seul ; exploitabilité sur ce site : `HYPOTHÈSE`). L'absence de CSP relève du hardening standard absent, non d'une vulnérabilité introduite par le thème.

## Résumé exécutif

Deux risques méritent une action avant mise en production : (1) l'absence de hash d'intégrité (SRI) sur la balise `<script>` jQuery injectée depuis `code.jquery.com` — si le CDN est compromis, du JavaScript arbitraire s'exécute sur toutes les pages sans aucune protection côté thème (`VÉRIFIÉ_CODE` : `functions.php:17-23`) ; (2) la version jQuery 1.12.4 est ancienne et associée à des vulnérabilités documentées pour XSS et prototype pollution dans les bases NVD/Mitre (source externe, non vérifiable depuis le dépôt seul ; exploitabilité sur ce site : `HYPOTHÈSE`). Le reste des constats (output WP correctement échappé, pas de secrets dans le code, pas de logique d'authentification dans le thème) est satisfaisant pour le périmètre versionné.

## Constats détaillés

**Absence de Subresource Integrity (SRI) sur jQuery CDN.** `functions.php:17-23` enregistre jQuery via `wp_enqueue_script()` vers l'URL `https://code.jquery.com/jquery-1.12.4.min.js` (`VÉRIFIÉ_CODE`). L'API native WordPress ne supporte pas nativement l'attribut `integrity` dans `wp_enqueue_script()` — il faudrait l'ajouter via un filtre `script_loader_tag` (selon la documentation WordPress, source externe non vérifiable depuis le dépôt). Aucun tel filtre n'est présent dans le dépôt (`VÉRIFIÉ_CODE` : `functions.php` lu intégralement). Conséquence : si `code.jquery.com` est compromis ou si la réponse CDN est altérée en transit (attaque man-in-the-middle sur un réseau sans HSTS), le navigateur exécutera le script altéré sans pouvoir le détecter. La liaison HTTPS réduit le risque de transit, mais ne protège pas contre une compromission côté serveur CDN.

**jQuery 1.12.4 — version ancienne avec vulnérabilités connues.** `VÉRIFIÉ_CODE` : `functions.php:19` contient l'URL `https://code.jquery.com/jquery-1.12.4.min.js`, confirmant que la version 1.12.4 est chargée sur toutes les pages. Selon les bases NVD/Mitre (source externe, non vérifiable depuis le dépôt seul) : jQuery 1.x est en fin de vie et des CVE couvrant cette version y sont documentés — CVE-2019-11358 (prototype pollution via `$.extend(true, …)`, jQuery < 3.4.0), CVE-2020-11022 et CVE-2020-11023 (XSS via html() et manipulation DOM, jQuery < 3.5.0), CVE-2015-9251 (XSS cross-domain via Ajax, jQuery < 3.0). Que ces vecteurs soient exploitables sur ce site dépend des usages jQuery faits par les plugins actifs (WooCommerce, Contact Form 7 — hors dépôt) : le niveau de risque réel est une `HYPOTHÈSE`.
...
- **jQuery 1.12.4** : version ancienne avec vulnérabilités documentées dans les bases NVD/Mitre (source externe, non vérifiable depuis le dépôt ; exploitabilité réelle : `HYPOTHÈSE`). Fichier : `functions.php:19`.
...
- **Exploitation de vulnérabilités jQuery connues** : des CVE couvrant la version 1.12.4 sont documentés dans les bases NVD/Mitre (source externe, non vérifiable depuis le dépôt). Leur exploitabilité dépend des usages faits par les plugins actifs (WooCommerce, Contact Form 7 — hors dépôt) : impact réel `HYPOTHÈSE`. Sévérité potentielle si exploités : XSS réfléchi ou stocké.

## Recommandations priorisées

1. **Mettre à jour jQuery vers 3.x** — migrer de `1.12.4` vers une version récente 3.x pour éliminer les CVE documentés. Fichier : `functions.php:19`. La version CDN à utiliser est listée sur `code.jquery.com` (source externe). Vérifier la compatibilité avec WooCommerce, Contact Form 7 et Yoast SEO (hors dépôt) avant déploiement.
2. **Ajouter un hash SRI sur la balise `<script>` jQuery** — via le filtre WordPress `script_loader_tag` dans `functions.php`, ajouter `integrity="sha384-..."` et `crossorigin="anonymous"` sur la balise émise. Le hash SRI de la version cible est obtenu depuis `code.jquery.com` ou `srihash.org` (sources externes). Cela protège contre la compromission CDN.
