...
2. **Appel `get_header()`** (`index.php:1`) — WordPress inclut `header.php` :
   ...
   - Chrome en-tête : `<header class="site-header">` avec, selon le contexte (`header.php:10-14`) :
     - **Vues liste (non singulières)** (`!is_singular()`) : `<h1>bloginfo('name')</h1>` — nom du site en H1
     - **Vues singulières (post, page)** : `<p class="site-name"><a href="esc_url(home_url('/'))">bloginfo('name')</a></p>` — lien vers l'accueil, sans H1
...
4. **Boucle WordPress** (`index.php:3-10`) :
   ...
   - **Si des posts existent** : pour chaque post, `the_post()` → rendu d'un `<article>` avec :
     ...
     - **Vues liste (non singulières)** : `<h2>` + `the_title()` (`index.php:5-9`) — titre affiché en H2 (H1 dans l'en-tête)
     - **Vues singulières (post, page)** : `<h1>` + `the_title()` (`index.php:5-9`) — titre affiché en H1 sémantique (H1 dans l'en-tête absent)
